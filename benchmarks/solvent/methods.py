...
def method_OPLS(OUTPUT_PATH, compound_ids, smiles_tags, experimental_v,solvent, SCHRODINGER_SRC, PLOPROTTEMP_SRC, PELE_SRC, IMPACT_TEMPLATE_PATH, ROTAMER_LIBRARY_PATH, VACUUM_CF, OBC_CF,VDGBNP_CF):
    energies = list()
    diff_list = list()
    exp_list = list()
    for cid, tag, exp_v in zip(compound_ids, smiles_tags, experimental_v):
        try:
            ...
            if solvent == 'OBC':
                ...
                print(IMPACT_TEMPLATE_PATH, ROTAMER_LIBRARY_PATH)
                print(os.getcwd())
...
def method_OFFOPLS(OUTPUT_PATH, compound_ids, smiles_tags, experimental_v, solvent, non_bonds, bonds_angles, CHARGES_METHOD, SCHRODINGER_SRC, PLOPROTTEMP_SRC, PELE_SRC, IMPACT_TEMPLATE_PATH, ROTAMER_LIBRARY_PATH, VACUUM_CF, OBC_CF,VDGBNP_CF, PATH_OPLS):
    energies = list()
    diff_list = list()
    exp_list = list()
    for cid, tag, exp_v in zip(compound_ids, smiles_tags, experimental_v):
        try: 
            ...
            # Handle non-bonding, nond and angle parameters 
            if non_bonds == 'OFF':
                # Generate its parameters and template file
                molecule.parameterize(OFF_FORCEFIELD, charges_method=CHARGES_METHOD)
